<template>
  <div id="puzzle" style="display:inline-block;">
    <div class="puzzle-container" :style="'margin-bottom:10px;padding:' + 10*scale + 'px ' + 10*scale + 'px ' + 28*scale + 'px;border-radius:'+5*scale+'px;'">
      ...
      <div class="re-btn" @click="refreshImg" :style="'height: '+28*scale+'px;padding: '+1*scale+'px '+16*scale+'px;font-size: '+18*scale+'px;'"><div/></div>
    </div>
    <div class="slider-container" :style="'width:' + dataWidth + 'px;'">
      <div class="slider-bar" :style="'border-radius:'+ 24*scale+'px;'">
        <p class="slider-bar-text select-none" onselectstart="return false" :style="'text-align:center;line-height:' + 28*scale + 'px;font-size:'+12*scale+'px;'">按住左边滑块， 拖动完成上方拼图</p>
      </div>
      <div class="slider-btn" ref="sliderBtn" @mousedown="startMove" @touchstart="startMove" :style="'top: '+ -7*scale + 'px;'"><a-icon type="pause-circle" theme="twoTone" twoToneColor="#52c41a" :style="'font-size: '+44*scale+'px;'" /></div>
    </div>
  </div>
</template>
...
.puzzle-container{position:relative;padding:16px 16px 28px;border:1px solid #ddd;background:white;border-radius:16px;}
.slider-container{position:relative;margin:auto;}
.slider-bar{border:1px solid #c3c3c3;border-radius:24px;background:#ebeced;box-shadow:0 1px 1px rgba(12,10,10,0.2) inset;}
.slider-bar-text{font-size:12px;color: #486c80;line-height:28px;margin:0;text-align:right;padding-left:22px;}
...
.slider-btn{position:absolute;width:44px;height:44px;left:0;top:-7px;z-index:12;cursor:pointer;background-image:url(../image/sprite.3.2.0.png);background-position:0 -84px;transition:inherit}
.ver-tips{position:absolute;left:0;bottom:-22px;background:rgba(255,255,255,.9);height:22px;line-height:22px;font-size:12px;width:100%;margin:0;text-align:left;padding:0 8px;transition:all .4s}
.slider-tips{bottom:0}
.ver-tips i{display:inline-block;width:22px;height:22px;vertical-align:top;background-image:url(../image/sprite.3.2.0.png);background-position:-4px -1229px}
.ver-tips span{display:inline-block;vertical-align:top;line-height:22px;color:#455}
.active-tips{display:block}
.hidden{display:none}
.re-btn{position:absolute;left:0;bottom:0;height:28px;padding:0 16px}
.re-btn div{display:inline-block;width:14px;height:14px;margin:7px 0;background-image:url(../image/sprite.3.2.0.png);background-position:0 -1179px;cursor:pointer}
.re-btn div:hover{background-position:0 -1193px}
</style>
